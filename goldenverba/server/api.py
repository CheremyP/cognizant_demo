from fastapi import FastAPI, WebSocket, status, File, UploadFile, HTTPException
...
def transcribe_audio(file_path= "temp_audio.wav"):
    ...
    for segment in segments:
        ...
        json.append({"start": segment.start, "end": segment.end, "text": segment.text})
...
async def transcribe(file: UploadFile = File(...)):
    UPLOAD_DIR = "uploads"
    if file is None or file.content_type != "audio/wav" or file.content_type != "audio/mpeg" or file.content_type != "audio/mp3":
        raise HTTPException(
    ...
    try:
        ...
        with open("temp_audio.wav", "wb") as buffer:
            ...
            buffer.write(contents)  # Write the contents to a temporary file
        
        # Transcribe the audio file
        transcription_result = transcribe_audio(audio_path= upload_name)
